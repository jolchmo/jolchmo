<!-- jolchmo's GitHub Profile README -->

<!-- Greeting with an engaging GIF -->
<h2> Hello there! I'm lch. 👋 </h2>

<!-- About Me Section -->
<h3> 🤖 About Me </h3>

- 🔭 &nbsp; I am currently focusing on **Robotics**, with a primary interest in Vision-Language-Action models (VLA) and their real-world applications.
- 🤔 &nbsp; I'm passionate about exploring ways to **boost productivity using AI**, from writing code and automating tasks to generating creative ideas.
- 💼 &nbsp; A robotics enthusiast, algorithm engineer, and a lifelong explorer of the AI frontier. I'm always eager to learn and build what's next.
- ✍️ &nbsp; In my spare time, I love to capture the world through my lens. **Photography and travel** are my go-to activities for inspiration and relaxation.
- ☕ &nbsp; Fueled by coffee and a curiosity for how things work. Let's connect and discuss technology, AI, or anything in between!

<!-- Tech Stack Section -->
<h3>🛠 Tech Stack & Tools</h3>

- 💻 &nbsp; **Programming Languages:** Python
- 🌐 &nbsp; **Robotics & AI:** ROS/ROS2 | PyTorch | MoveIt | Gazebo | OpenCV | LangChain
- 🛢 &nbsp; **Data & Backend:** | Docker | BeautifulSoup | requests |  Dify | PostgreSQL
- 🔧 &nbsp; **Development Tools:** VS Code | Git | GitHub | Jupyter Notebook | Linux (Ubuntu)

<br>

<!-- GitHub Stats -->
<!-- <p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=jolchmo&include_all_commits=true&count_private=true&show_icons=true&theme=dark" alt="jolchmo's Github Stats"/>
  <!-- <br><br> -->
  <!-- <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=jolchmo&layout=compact&text_color=daf7dc&bg_color=151515" alt="Top Languages"/> -->
</p>  -->

<!-- Social Links / Connect with Me -->
<h3> 🤝🏻 Connect with Me </h3>


<p>
  Feel free to reach out for collaborations, questions, or just a friendly chat!
</p>


<p align="center">
  <br><br>
  <!-- Replace the [ ] links with your own links -->
  <a href="mailto:jolchmo@gmail.com" target="_blank" rel="noopener noreferrer"><img src="https://img.icons8.com/plasticine/100/000000/gmail.png" width="50" /></a>
  &nbsp;
  <a href="https://t.me/Jzxtd" target="_blank"><img src="https://img.icons8.com/plasticine/100/000000/telegram-app.png" width="50" alt="Telegram"/></a>
</p>

<!-- Buy Me a Coffee / Support Me Section -->
<h3> ☕️ Buy Me a Coffee </h3>

<p>
  If you find my projects helpful or just want to support my work, feel free to buy me a coffee! Your support is greatly appreciated and helps me dedicate more time to open-source contributions.
</p>


<p>
  <img src="https://imgbed-hk.oss-cn-hongkong.aliyuncs.com/img/sol.png" width="20" style="vertical-align: middle; margin-right: 5px;" />
  <strong>SOL Address:</strong> <code>HFaGAh5UFaPX3v14kh3f3GqZXA2vUzJ7jkC7BYcnDAWa</code>
  <strong>SOL Domain:</strong> <code>jolchspace.sol</code>
  
</p>

